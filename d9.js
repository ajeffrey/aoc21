...
function lowestNeighbour(map, [x, y]) {
  ...
  return neighbours.reduce(
    ([ax, ay], [bx, by]) => (map[ay][ax] < map[by][bx] ? [ax, ay] : [bx, by]),
    [x, y]
  );
}
...
function computeBasins(map) {
  ...
  for (let y = 0; y < map.length; y++) {
    ...
    for (let x = 0; x < row.length; x++) {
      ...
      do {
        ...
        coords[0] !== prevCoords[0][0] ||
        coords[1] !== prevCoords[0][1]
      );

      const basin = (basins[coords.join(",")] =
        basins[coords.join(",")] || new Set());
  ...
  const topBasins = Object.values(basins)
...
const map = require("fs")
  .readFileSync("inputs/d9.txt", "utf-8")
  .split("\n")
