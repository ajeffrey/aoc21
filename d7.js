function alignCrabs(positions) {
  ...
  for(let i = min; i <= max; i++) {
    const posTotal = positions.map(p => Math.abs(p - i)).reduce((a, b) => a + b);
...
function alignCrabsPoly(positions) {
  ...
  let total = Infinity;
  for(let i = min; i <= max; i++) {
    const posTotal = positions.map(p => {
...
const positions = require('fs').readFileSync('inputs/d7.txt', 'utf-8').split(',').map(n => parseInt(n, 10));
